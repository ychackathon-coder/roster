...
function Brain({ progressRef }: Props) {
  ...
  useFrame((state, delta) => {
    ...
    // Continuous slow spin, with a scrubbed tilt across the track.
    group.current.rotation.y += delta * 0.09;
    group.current.rotation.x = -0.22 + Math.sin(progress * Math.PI) * 0.28;

    // Push in through the middle of the track, pull back before release,
    // keeping the whole sphere in frame the entire way.
